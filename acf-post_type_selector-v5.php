...
class acf_field_post_type_selector extends acf_field {
	...
	*/
	...
	function __construct() {
		...
		*  var message = acf._e('post_type_selector', 'error');
		*/
	...
	function render_field_settings( $field ) {
		...
		*/
		...
		acf_render_field_setting( $field, array(
			'label'			=> __('Layout','acf-post_type_selector'),
			'type'			=> 'select',
			'name'			=> 'select_type',
			'layout'        => 'horizontal',
			'choices'       => array( 
		...
		switch ( $field[ 'select_type' ] ) {
			...
			case acf_field_post_type_selector::SELECTOR_TYPE_RADIO:
				...
				?>
					...
					<li><input type="radio" <? echo ( isset( $checked[ $post_type->name ] ) ) ? $checked[ $post_type->name] : null; ?> class="<? echo $field[ 'class' ]; ?>" name="<? echo $field[ 'name' ]; ?>" value="<? echo $post_type->name; ?>"><label><? echo $post_type->labels->name; ?></label></li>
			...
			case acf_field_post_type_selector::SELECTOR_TYPE_CHECKBOXES:
				...
				echo '<ul class="acf-checkbox-list acf-hl checkbox">';
				...
				?>
					...
					<li>
						<input type="checkbox" <? echo isset($checked[ $post_type->name ])?$checked[$post_type->name]:null; ?> class="<? echo $field['class' ]; ?>" name="<? echo $field['name']; ?>[]" id="<? echo $field['name']; ?>[<? echo $post_type->labels->name; ?>]" value="<? echo $post_type->name; ?>">
						<label for="<? echo $field['name']; ?>[<? echo $post_type->labels->name; ?>]">
	...
	*/
	...
	function input_admin_enqueue_scripts() {
		...
		// register & include JS
		wp_register_script( 'acf-input-post_type_selector', "{$dir}js/input.js" );
		wp_enqueue_script('acf-input-post_type_selector');
		
		
		// register & include CSS
		wp_register_style( 'acf-input-post_type_selector', "{$dir}css/input.css" ); 
	...
	*/
	...
	function validate_value( $valid, $value, $field, $input ){
		...
		if( $value < $field['custom_minimum_setting'] )
		{
			$valid = __('The value is too little!','acf-post_type_selector'),
		}
